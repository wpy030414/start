...
5. **设置气泡（背景循环）**：左下角图标，开关「循环播放」（图片 6s、视频播完切换）与「循环类型隔离」（只切同类型）；「重置」仅清搜索历史与背景。
...
- 数据库名 `homepage_bg`（版本 3，单一对象仓库 `background`），对象仓库名 `background`。
- 仅保存两类键：
  - `hash -> File`：通过 `bgHash(file)` 生成，哈希依据 `文件名 | 大小 | 类型 | 最后修改时间`（FNV-1a 风格，base32 输出）。
...
- 18 列 × 14 行，从顶部均匀排布到 2/3 vh，越向下越小，到 2/3 vh 处缩为 0 彻底消失；隔行错位半格形成波点排布。
...
- 设置项持久化在 localStorage，默认全部关闭：
  - `homepage_bg_cycle`（循环播放）：开启后在已设置的背景间循环切换。
  - `homepage_bg_cycle_type_isolated`（循环类型隔离）：开启后只在与当前项同类型（图 / 视频）的背景间切换。
- 切换节奏：图片每 6 秒切换一次；视频设 `loop=false` 并等待 `ended` 事件后再切换。
- 候选集合由 `cycleCandidateIndices()` 计算：隔离时过滤同类型下标；候选不足 2 个则保持静态（视频恢复 `loop=true`）。
- 每次切换 / 手动 `← / →` / 添加 / 置顶后都经 `setBackground()` 末尾的 `scheduleBgCycle()` 重新排程，手动导航会重置计时。
- 「重置」按钮：仅清除搜索历史（`homepage_search_history`）与 IndexedDB 背景数据（`background` 仓 `clear()`），保留快捷链接、默认引擎与设置项，随后 `location.reload()`；点击时 `confirm()` 二次确认。
...
- 最多保存 6 个快捷链接（`MAX_SHORTCUTS`），存在 `localStorage` 的 `homepage_shortcuts` 中。
...
- 对主文档采用 stale-while-revalidate：
  - 缓存命中且 < 24h（时间戳存于 IndexedDB `sw_main_meta.cacheMeta`）→ 立即返回，后台异步刷新；
  - 缓存缺失或过期 → 取网络，失败则回退缓存，再回退离线备份。
